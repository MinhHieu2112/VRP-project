...
def load_data(locations_file, matrix_file):
    ...
    for idx, row in locations_df.iterrows():
        node = Node(id=int(row['id']), x=float(row['lat']), y=float(row['lon']), demand=1)  # demand=1 as per config
        nodes.append(node)
...
def run_aco_solver():
    ...
    standardized_result = {
        ...
        "execution_time": 0.0,  # ACO doesn't track execution time internally
        "routes": routes_dict,
    ...
    # Visualize
    # Load locations data for visualizer
    locations_df = pd.read_csv(locations_file)
    visualizer = Visualizer(locations_df)
    map_path = os.path.join(aco_output_dir, 'aco_route_map.html')
    visualizer.draw(routes_dict, map_path)

    print(f"ACO Result: Distance {best_distance:.2f}, Vehicles {best_vehicles}")
