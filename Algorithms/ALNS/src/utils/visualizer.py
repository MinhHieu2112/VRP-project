...
class Visualizer:
    ...
    def _check_osrm_health(self):
        ...
        try:
            q = f"{self.osrm_url}/route/v1/driving/{self.depot_coords[1]},{self.depot_coords[0]};{self.depot_coords[1]},{self.depot_coords[0]}?overview=false"
            r = requests.get(q, timeout=2)
            data = r.json()
            if r.status_code == 200 and 'routes' in data:
                return True
        except Exception as e:
            print(f"[VISUALIZER] OSRM health check failed ({e}), fallback line thẳng.")
        return False
    ...
    def _get_route(self, p1, p2):
        ...
        try:
            ...
            print(f"[VISUALIZER] OSRM route request failed ({e}), fallback line thẳng.")
