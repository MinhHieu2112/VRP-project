...
# Thêm hàm này vào cuối file destroy_operators.py
def shaw_removal(state, random_state):
    """Xóa các khách hàng có sự tương đồng về vị trí (gần nhau)."""
    destroyed = state.copy()
    num_to_remove = get_nodes_to_remove(state)
    
    # Lấy danh sách khách hàng hiện có
    all_clients = []
    for route in destroyed.routes:
        all_clients.extend([n for n in route if n != 0])
    
    if not all_clients: return destroyed

    # Chọn 1 khách hàng ngẫu nhiên làm tâm
    pivot = random_state.choice(all_clients)
    
    # Tính khoảng cách từ pivot đến các khách hàng khác
    distances = []
    for node in all_clients:
        dist = state.distance_matrix[pivot, node]
        distances.append((dist, node))
    
    # Sắp xếp và chọn các node gần nhất để xóa
    distances.sort(key=lambda x: x[0])
    to_remove = [d[1] for d in distances[:num_to_remove]]
