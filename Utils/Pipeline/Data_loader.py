...
class DataLoader:
    ...
    def load_data(self) -> dict:
        ...
        # Lấy num_points từ config (mặc định lấy tất cả nếu không có)
        model_params = self.config.get('common_model_parameters', {})
        num_points = model_params.get('num_points', None)
        
        matrix_int       = self._load_matrix(num_points)
        df_locs, demands = self._load_locations(default_demand, num_points)
    ...
    def _load_matrix(self, num_points: int = None) -> np.ndarray:
        ...
        if num_points is not None:
            # Cắt ma trận, +1 vì index 0 là depot
            limit = num_points + 1
    ...
    def _load_locations(self, default_demand: int, num_points: int = None):
        ...
        if num_points is not None:
            # Cắt danh sách tọa độ, +1 vì tính cả depot
            limit = num_points + 1
            df = df.head(limit)
